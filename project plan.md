...
### **阶段 0: 基础架构与原型验证 (预计时间: 1-2周) [已完成]**

**目标**: 搭建项目骨架，并验证 `libgit2` 与 SwiftUI 的可行性。本阶段结束时，应有一个能打开本地仓库并打印提交历史到控制台的程序。

* [x] **任务 0.1: 项目初始化**
  * [x] 创建一个新的 macOS SwiftUI 应用项目。
  * [x] 初始化 Git 仓库，并建立基本的项目文件结构。

* [x] **任务 0.2: Git 后端服务层集成**
  * [x] 使用 SPM 引入一个 `libgit2` 的 Swift 封装库（例如，研究并选择一个社区维护的库，或者自己创建一个简单的封装）。
  * [x] 创建一个 `GitService` 类，作为与 `libgit2` 交互的唯一入口。
  * [x] 在 `GitService` 中实现第一个核心功能：`openRepository(at path: String)`。
  * [x] 实现 `fetchCommitHistory()` 方法，该方法能返回一个 `Commit` 对象的数组 (自定义的 Swift 结构体)。

* [x] **任务 0.3: 架构与 UI 原型**
  * [x] 设计并创建核心的 Model 对象 (例如 `Repository`, `Commit`, `Branch`, `FileStatus`)。
  * [x] 创建主 `MainViewModel`，负责管理当前打开的仓库状态。
  * [x] 在 SwiftUI `ContentView` 中，添加一个 "Open Repository" 按钮。
  * [x] 实现点击按钮后，调用 ViewModel，ViewModel 再调用 `GitService`，并将获取到的提交历史**打印到 Xcode 控制台**。

---

### **阶段 1: 核心只读功能 (预计时间: 2-3周) [部分完成]**

**目标**: 实现完整的仓库浏览和差异查看功能。本阶段结束时，用户应能像使用一个 "Git 查看器" 一样使用本应用。

* [x] **任务 1.1: 仓库管理与导航**
  * [x] 设计一个欢迎界面，用于打开或克隆仓库。
  * [x] 实现一个侧边栏，用于展示和切换已打开的仓库。
  * [x] 实现应用状态持久化（例如，记住上次打开的仓库）。

* [x] **任务 1.2: 左侧功能列表重构与Git工作流实现**

  **设计理念**: 移除传统的导航栏，采用功能导向的列表设计，左侧为Git功能列表，右侧为动态内容区域。其他UI布局保持不变（欢迎页面和Tab页面功能已完成）。

  **子任务 1.2.1: 左侧功能列表基础架构**
  * [x] **移除传统导航栏**: 重构现有的侧边栏组件
    * [x] 移除原有的导航栏设计
    * [x] 保持欢迎页面和Tab页面的现有布局不变
    * [x] 确保仓库视图的右侧主体区域保持不变
  * [x] **功能列表框架**: 创建FunctionListView组件
    * [x] 设计两层功能列表结构（固定选项 + 可展开列表）
    * [x] 实现功能项点击状态管理
    * [x] 创建功能列表与右侧内容区域的通信机制
    * [x] 实现响应式布局和列表项样式设计

  **子任务 1.2.2: 下部分可展开功能列表实现**
  * [ ] **"本地分支"可展开列表**:
  ...
  **子任务 1.2.3: 上部分固定功能选项实现**
  * [x] **"本地修改"功能实现**:
    * [x] 创建LocalChangesView组件（右侧显示区域暂时留白）
    * [x] 实现点击"本地修改"后右侧页面切换逻辑
    * [x] 预留本地修改状态的数据接口
    * [x] 设计本地修改功能的占位界面
  * [x] **"Stash列表"功能实现**:
    * [x] 创建StashListView组件（右侧显示区域暂时留白）
    * [x] 实现点击"Stash列表"后右侧页面切换逻辑
    * [x] 预留Stash数据获取的接口设计
    * [x] 设计Stash列表功能的占位界面
